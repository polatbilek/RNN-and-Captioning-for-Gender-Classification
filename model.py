...
class network(object):
	...
	def __init__(self):
		...
		# placeholders
		self.textrnn_input = tf.placeholder(tf.float32, [FLAGS.batch_size, FLAGS.textrnn_size])
		self.textcnn_input = tf.placeholder(tf.float32, [FLAGS.batch_size, FLAGS.textcnn_size])
		self.imagernn_input = tf.placeholder(tf.float32, [FLAGS.batch_size, FLAGS.imagernn_size])
		...
		self.weights = {'fc1': tf.Variable(tf.random_normal([FLAGS.textrnn_size, FLAGS.num_classes]), name="fc1-weights")}
		# biases
	...
	def architecture(self):
		...
		self.concatenated_input = self.textrnn_input
