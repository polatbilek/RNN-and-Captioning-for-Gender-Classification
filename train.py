	...
	with tf.Session(config=tf.ConfigProto(allow_soft_placement=True)) as sess:
		...
		for epoch in range(FLAGS.num_epochs):
			...
			for batch in range(training_batch_count):
				#prepare the batch
				training_batch_textrnn, training_batch_textcnn, training_batch_imagernn, training_batch_users, training_batch_y = \
			...
			for batch in range(valid_batch_count):
				...
				#prepare the batch
				valid_batch_textrnn, valid_batch_textcnn, valid_batch_imagernn, valid_batch_users, valid_batch_y = \
										prepVectorBatchData(valid_textrnn_vectors, valid_textcnn_vectors, valid_imagernn_vectors, valid_users, target_values, batch)


				#run the graph
				feed_dict = {network.textrnn_input: valid_batch_textrnn, network.textcnn_input: valid_batch_textcnn, network.imagernn_input: valid_batch_imagernn, \
							network.Y: training_batch_y, network.reg_param: FLAGS.l2_reg_lambda}
