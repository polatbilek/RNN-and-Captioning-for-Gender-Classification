...
def train(network, target_values):
    ...
    with tf.Session() as sess:
        ...
        for epoch in range(FLAGS.num_epochs):
            ...
            random.shuffle(c)
            usernames, target_batches = zip(*c)

            training_index = (len(training_users) * FLAGS.training_set_size)
            ...
            batch_accuracy = 0.0
            batch_loss = 0.0
            training_batch_count = int(len(training_users) / FLAGS.batch_size)
            valid_batch_count = int(len(valid_users) / FLAGS.batch_size)
